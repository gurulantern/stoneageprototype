...
public class NetworkedEntity : MonoBehaviour
{
    ...
    [SerializeField]
    private CharControllerMulti movement = null;
...
*/
    ...
    protected virtual void ProcessViewSync()
    {
        ...
        double interpolationTime = ColyseusManager.Instance.ServerTime - interpolationBackTimeMs;
        ...
        if (proxyStates[0].timestamp > interpolationTime)
        {
            ...
            float deltaFactor = (ColyseusManager.Instance.ServerTime > proxyStates[0].timestamp) ?
                (float)(ColyseusManager.Instance.ServerTime - proxyStates[0].timestamp) * 0.2f : 0f;
